    ...
    <style>
        ...
        * {
        ...
        .badge-admin {
            ...
            color: #f07c5a;
            border: 1px solid rgba(232, 56, 13, .3);
        }
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('historial.index') }}"
                class="nav-item {{ request()->routeIs('historial*') ? 'active' : '' }}" onclick="cerrarSidebar()">
                <span class="nav-icon">📋</span> Historial
            </a>
            @endif
            @if(auth()->user()->puede('CU06_BUS'))
            <a href="{{ route('proforma.index') }}"
                class="nav-item {{ request()->routeIs('proforma.*') ? 'active' : '' }}" onclick="cerrarSidebar()">
            ...
            {{-- Administración --}}
            @if(auth()->user()->puede('CU13_BUS') || auth()->user()->puede('CU13_PRI') ||
            auth()->user()->puede('CU21_BUS'))
            <div class="nav-section">Administración</div>
            @if(auth()->user()->puede('CU13_BUS'))
            <a href="{{ route('usuarios.index') }}"
                class="nav-item {{ request()->routeIs('usuarios*') || request()->routeIs('cargos*') ? 'active' : '' }}"
                onclick="cerrarSidebar()">
            ...
            <a href="{{ route('permisos.index') }}"
                class="nav-item {{ request()->routeIs('permisos*') ? 'active' : '' }}" onclick="cerrarSidebar()">
            ...
            <a href="{{ route('contratos.index') }}"
                ...
                <span class="nav-icon">📄</span> Contratos Laborales
            </a>
            @endif
            @if(auth()->user()->puede('CU12_BUS'))
        ...
        <div class="sidebar-footer">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="btn btn-ghost" style="width:100%;justify-content:center;">↩ Cerrar
                    sesión</button>
            </form>
        </div>
    </aside>
