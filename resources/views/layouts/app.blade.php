{{-- REEMPLAZA COMPLETAMENTE resources/views/layouts/app.blade.php --}}
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        :root{--bg:#0f1117;--surface:#181c27;--surface2:#1f2436;--border:#2a3045;--accent:#f5a623;--text:#e8eaf0;--muted:#6b7591;--success:#2ecc71;--danger:#e74c3c;--radius:6px;--sidebar-w:220px;}
        *{box-sizing:border-box;margin:0;padding:0;}
        body{font-family:'Barlow',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;display:flex;}
        .sidebar{width:var(--sidebar-w);background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;position:fixed;top:0;left:0;bottom:0;z-index:200;transition:transform .25s ease;}
        ...
        .hamburger{display:none;background:transparent;border:1px solid var(--border);border-radius:var(--radius);color:var(--muted);cursor:pointer;padding:.4rem .6rem;font-size:1.1rem;line-height:1;transition:color .15s,border-color .15s;flex-shrink:0;}
        ...
        .btn-primary{background:var(--accent);color:#111;}.btn-primary:hover{background:#ffc04a;transform:translateY(-1px);}
        ...
        .btn-danger{background:rgba(231,76,60,.12);color:var(--danger);border:1px solid rgba(231,76,60,.25);}.btn-danger:hover{background:rgba(231,76,60,.22);}
        ...
        .field-group .req{color:var(--accent);}.field-group .hint{font-size:.7rem;font-weight:400;text-transform:none;letter-spacing:0;opacity:.65;}
        ...
        .form-errors ul{padding-left:1.2rem;}.form-errors li{font-size:.875rem;color:var(--danger);margin-top:.2rem;}
        ...
        table{width:100%;border-collapse:collapse;font-size:.875rem;min-width:500px;}
        ...
        @media(max-width:480px){.stats-grid{grid-template-columns:1fr;}.topbar-title{font-size:1rem;}}
    </style>
    @stack('styles')
</head>
<body>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        {{-- Administración --}}
        @if(auth()->user()->puede('CU13_BUS') || auth()->user()->puede('CU13_PRI') || auth()->user()->puede('CU21_BUS'))
        <div class="nav-section">Administración</div>
        @if(auth()->user()->puede('CU13_BUS'))
        ...
        <a href="{{ route('roles.index') }}" class="nav-item {{ request()->routeIs('roles*') ? 'active' : '' }}" onclick="cerrarSidebar()">
        ...
        <a href="{{ route('permisos.index') }}" class="nav-item {{ request()->routeIs('permisos*') ? 'active' : '' }}" onclick="cerrarSidebar()">
            <span class="nav-icon">🔐</span> Privilegios
        </a>
        @endif
        ...
        @if(auth()->user()->puede('CU04_ADD') || auth()->user()->puede('CU14_BUS') || auth()->user()->puede('CU09_BUS'))
        ...
        <a href="{{ route('prestamo.index') }}"
            class="nav-item {{ request()->routeIs('prestamo*') ? 'active' : '' }}"
            onclick="cerrarSidebar()">
    ...
    <div class="sidebar-footer">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="btn btn-ghost" style="width:100%;justify-content:center;">↩ Cerrar sesión</button>
        </form>
    </div>
</aside>
...
<script>
function abrirSidebar(){document.getElementById('sidebar').classList.add('open');document.getElementById('sidebar-overlay').classList.add('active');document.body.style.overflow='hidden';}
function cerrarSidebar(){document.getElementById('sidebar').classList.remove('open');document.getElementById('sidebar-overlay').classList.remove('active');document.body.style.overflow='';}
